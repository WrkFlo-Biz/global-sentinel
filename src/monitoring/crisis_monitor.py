...
class CrisisMonitor:
    ...
    def __init__(self, repo_root: Path):
        ...
        try:
            # DISABLED: OpenClaw gateway handles bot polling now
    ...
    def _run_cycle(self):
        ...
        freshness_result = self._check_feature_freshness(bridge_results)

        # 4. Score regime shift (with freshness-aware confidence penalty)
        regime_score = self._score_regime(snapshot)
        if freshness_result and freshness_result.get("max_confidence_penalty", 0) > 0:
            original_confidence = regime_score.get("confidence", 0.0)
            penalty = freshness_result["max_confidence_penalty"]
            regime_score["confidence"] = max(0.0, original_confidence * (1.0 - penalty))
            regime_score["_freshness_penalty_applied"] = penalty
        ...
        scorecard = {
            ...
            "degraded_mode": freshness_result.get("active_degraded_groups", freshness_result.get("degraded_groups", 0)) > 0 if freshness_result else False,
        ...
        try:
            ...
            # Write GSS timeline point (every cycle, not just non-NEUTRAL)
            try:
                import json as _json
                gss_dir = self.repo_root / "logs" / "gss"
                gss_dir.mkdir(parents=True, exist_ok=True)
                timeline_point = {
                    "timestamp_utc": iso_now(),
                    "gss_signal": gss_result.get("gss_signal", "NEUTRAL"),
                    "z_score": gss_result.get("field_data", {}).get("z_score"),
                    "narrative_velocity": gss_result.get("narrative_data", {}).get("velocity"),
                    "vix": gss_result.get("execution_data", {}).get("vix"),
                    "confidence": gss_result.get("confidence"),
                    "action": gss_result.get("action"),
                }
                # Append to timeline
                with open(gss_dir / "signal_timeline.jsonl", "a") as tf:
                    tf.write(_json.dumps(timeline_point) + "\n")
                # Write latest
                with open(gss_dir / "latest_signal.json", "w") as lf:
                    _json.dump(gss_result, lf, default=str)
                # Trim timeline to last 500 points
                tl_path = gss_dir / "signal_timeline.jsonl"
                lines = tl_path.read_text().strip().split("\n")
                if len(lines) > 500:
                    tl_path.write_text("\n".join(lines[-500:]) + "\n")
            except Exception as _tl_err:
                print(f"[{iso_now()}] GSS timeline write error: {_tl_err}", file=sys.stderr)
        except Exception as e:
        ...
        # Quantum disabled — QPanda local simulator uses ~1GB RAM, causes OOM on 2GB VM
        # Re-enable when VM is upgraded or using cloud quantum backend only
        # try:
    ...
    def _check_feature_freshness(self, bridge_results: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Recalculate penalties excluding groups where no features have
            # active bridges (all features are "missing"). Those groups
            # represent undeployed bridges, not stale data.
            active_penalties = []
            active_degraded = 0
            for _gname, ginfo in summary.get("groups", {}).items():
                has_active = ginfo.get("fresh", 0) + ginfo.get("stale", 0) > 0
                if has_active:
                    active_penalties.append(ginfo.get("confidence_penalty", 0.0))
                    if ginfo.get("degraded"):
                        active_degraded += 1
            if active_penalties:
                summary["max_confidence_penalty"] = max(active_penalties)
            else:
    ...
    def _poll_bridges(self) -> Dict[str, Any]:
        ...
        # DISABLED 2026-03-21: aviation_disruption bridge - zero trade value
        # try:
        ...
        # DISABLED 2026-03-21: gcp_consciousness bridge - zero trade value
        # try:
    ...
    def _route_dual_strategy(
        ...
        for strategy_name, ideas in split.items():
            ...
            # EOD guard: block day_trade after 15:40 ET or when flatten flag is active
            if strategy_name == "day_trade" and (self.repo_root / "control" / "eod_flatten_active").exists():
                self._log_event("day_trade_eod_flag_blocked", {})
                continue
            if strategy_name == "day_trade":
                try:
                    from zoneinfo import ZoneInfo as _ZI
                    _et = __import__("datetime").datetime.now(_ZI("America/New_York"))
                    if _et.weekday() < 5 and (_et.hour > 15 or (_et.hour == 15 and _et.minute >= 40)):
                        print(f"[EOD] Blocking day_trade orders at {_et}"); continue
