#!/usr/bin/env python3
"""
Global Sentinel V4.5 - Treasury / OFAC Bridge (Skeleton)

Purpose:
- Poll Treasury press releases page and OFAC sanctions/actions information pages
- Emit normalized macro_policy_event packets for:
  - treasury_sanctions_or_regulatory_action
  - executive_policy_action (rare, if clearly identified)
  - macro_calendar_update
  - official_release_schedule_change (heuristic page change detection)
...
def read_text_url(url: str, timeout: int = 20) -> str:
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "GlobalSentinelTreasuryOFACBridge/1.0 (+shadow-mode)"}
    )
...
def find_links(html: str, base_url: str) -> List[Tuple[str, str]]:
    """Lightweight link extractor. Returns list of (href, anchor_text)."""
    out = []
...
# Bridge
# -----------------------------
class TreasuryOFACBridge:
    def __init__(self, repo_root: Path):
        self.repo_root = repo_root
        self.cfg = load_yaml(repo_root / "config" / "macro_policy_intel.yaml")
        self.macro_cfg = self.cfg.get("macro_policy_intel", {})
        self.treasury_cfg = (self.macro_cfg.get("official_sources", {}) or {}).get("treasury", {})
        self.ofac_cfg = (self.macro_cfg.get("official_sources", {}) or {}).get("ofac", {})
        ...
        self.cache_dir = repo_root / "logs" / "bridge_cache" / "treasury_ofac_bridge"
        self.cache_dir.mkdir(parents=True, exist_ok=True)

        self.pages: List[Tuple[str, str, str]] = []  # (source_group, page_name, url)
        treasury_pages = self.treasury_cfg.get("pages", {}) or {}
        ofac_pages = self.ofac_cfg.get("pages", {}) or {}
        for name, url in treasury_pages.items():
            self.pages.append(("treasury", name, url))
        for name, url in ofac_pages.items():
            self.pages.append(("ofac", name, url))
    ...
    def poll(self) -> List[Dict[str, Any]]:
        ...
        for source_group, page_name, url in self.pages:
            try:
    ...
    # Page polling
    # -------------------------
    def _poll_page(self, source_group: str, page_name: str, url: str) -> List[Dict[str, Any]]:
        ...
        # Content fingerprint -> page-change signal
        content_hash = sha1_hex(text[:8000])
        cache_file = self.cache_dir / f"{source_group}_{page_name}_hash.txt"
        ...
        if changed:
            packets.append(
                self._make_packet(
                    ...
                    headline=f"{source_group.upper()} page changed: {page_name}",
                    summary=f"Detected content hash change on {source_group} page '{page_name}'. Re-validate sanctions/policy alerts.",
                    parsing_meta={
                        "page_name": page_name,
                        "page_title": title,
        ...
        # Page heartbeat packet
        page_event_type = "macro_calendar_update"
        if source_group == "ofac":
            page_event_type = "treasury_sanctions_or_regulatory_action"
        ...
        packets.append(
            self._make_packet(
                ...
                headline=f"{source_group.upper()} page polled: {title or page_name}",
                summary=f"Official {source_group.upper()} page '{page_name}' polled successfully.",
                parsing_meta={
                    "page_name": page_name,
                    "page_title": title,
        ...
        # Item-level packets from headline links
        packets.extend(self._extract_item_packets(source_group, page_name, url, title, links))

        return packets

    def _extract_item_packets(
        self,
        source_group: str,
        page_name: str,
        page_url: str,
        ...
        for href, txt in links:
            ...
            if source_group == "treasury":
                if any(k in t for k in ["press release", "treasury", "secretary", "sanctions", "statement", "actions"]):
                    filtered.append((href, txt))
                elif "/news/press-releases/" in h:
                    filtered.append((href, txt))
            elif source_group == "ofac":
                if any(k in t for k in ["sanction", "ofac", "designation", "general license", "faq", "russia", "iran", "shipping", "vessel"]):
                    filtered.append((href, txt))
                elif "ofac" in h or "sanctions" in h:
                    filtered.append((href, txt))
        ...
        for href, txt in filtered:
            ...
            event_type, parsing_meta = self._classify_item(source_group, txt, href)
            packets.append(
                self._make_packet(
                    source_group=source_group,
                    source_url=href,
                    ...
                    headline=f"{source_group.upper()} linked item: {txt or href}",
                    summary=f"Discovered item link on {page_name}.",
                    parsing_meta={
                        "page_name": page_name,
    ...
    # Classification
    # -------------------------
    def _classify_item(self, source_group: str, headline: str, url: str) -> Tuple[str, Dict[str, Any]]:
        ...
        if source_group == "ofac":
            if any(k in text for k in ["sanction", "designation", "ofac", "general license", "faq"]):
                return "treasury_sanctions_or_regulatory_action", {"matched_keywords": ["ofac/sanctions heuristic"]}

        if source_group == "treasury":
            if any(k in text for k in ["sanctions", "ofac", "designation"]):
                return "treasury_sanctions_or_regulatory_action", {"matched_keywords": ["treasury sanctions heuristic"]}
        ...
        if source_group in {"treasury", "ofac"}:
            return "treasury_sanctions_or_regulatory_action", {"matched_keywords": []}
    ...
    def _make_packet(
        ...
        urgency = float(urgency_defaults.get(event_type, 0.50))

        rate_sensitive_keywords = [
            safe_lower(k) for k in (((self.parsing_rules.get("rate_sensitive_topic_keywords") or {}).get("includes_any")) or [])
        ]
        text = f"{safe_lower(headline)} {safe_lower(summary)}"
        keyword_rate_sensitive = any(k in text for k in rate_sensitive_keywords)

        rate_regime = False
        if event_type in {"treasury_sanctions_or_regulatory_action", "executive_policy_action"}:
            rate_regime = keyword_rate_sensitive or any(k in text for k in ["sanction", "tariff", "oil", "energy", "trade", "iran", "russia", "shipping"])
        ...
        event_id_input = f"{source_group}|{source_url}|{headline}|{published_time_utc or ''}|{json.dumps(parsing_meta, sort_keys=True)}"
        event_id = f"{source_group}-{sha1_hex(event_id_input)[:16]}"

        domain = "home.treasury.gov" if source_group in {"treasury", "ofac"} else "unknown"
    ...
    def _error_packet(self, source_url: str, error: str, page_name: Optional[str], source_group: str) -> Dict[str, Any]:
        return {
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Global Sentinel Treasury/OFAC Bridge")
    ...
    p.add_argument("--loop-seconds", type=int, default=300, help="Polling interval for loop mode")
    return p.parse_args()
...
def main() -> None:
    ...
    bridge = TreasuryOFACBridge(repo_root)
