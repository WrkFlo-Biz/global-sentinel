#!/usr/bin/env python3
"""Adapter for CFTC Commitments of Traders bridge."""
...
class CFTCAdapter(BaseBridge):
    source = "cftc"
    source_tier = "tier_1_official"
    trust_weight = 0.85
    freshness_ttl_minutes = 1440  # weekly data
    ...
    def _get_inner(self):
        if self._inner is None:
            from src.ingestion.cftc_bridge import CFTCBridge
            self._inner = CFTCBridge()
        return self._inner
    ...
    def fetch(self) -> Dict[str, Any]:
        try:
            ...
            logger.warning("CFTCAdapter fetch failed: %s", e)
