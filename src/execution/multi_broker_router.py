...
def _check_ibkr_health() -> BrokerHealth:
    """Check IBKR balance via ib_async connection to localhost:4001."""
    h = BrokerHealth()
    h.last_check = time.time()
    try:
        import asyncio
        ...
        async def _probe():
            ib = IB()
            await ib.connectAsync("127.0.0.1", 4001, clientId=99, timeout=8)
        ...
        h.connected = True
        h.buying_power = float(data.get("BuyingPower", 0))
        h.equity = float(data.get("NetLiquidation", 0))
        h.day_trades_remaining = 999
        h.extra = {
            "available_funds": data.get("AvailableFunds", "0"),
            "total_cash": data.get("TotalCashValue", "0"),
        }
    except Exception as e:
        h.error = str(e)[:200]
    return h
...
def _execute_ibkr(symbol: str, qty: int, side: str, order_type: str,
    ...
    """Execute trade via IBKR ib_async."""
    try:
        import asyncio
        ...
        async def _place():
            ib = IB()
            await ib.connectAsync("127.0.0.1", 4001, clientId=100, timeout=10)
            ...
            action_str = "BUY" if side == "buy" else "SELL"
            if order_type == "limit" and limit_price:
                order = LimitOrder(action_str, abs(qty), limit_price)
            else:
                order = MarketOrder(action_str, abs(qty))
        ...
        loop = asyncio.new_event_loop()
        try:
            return loop.run_until_complete(_place())
        finally:
            loop.close()
