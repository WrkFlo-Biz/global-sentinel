...
def request_approval(order_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Read auto_execute early so error paths can respect it
    ...
    if not token or not chat_id:
        ...
        result = {"approved": auto_execute, "decision": "error", "reason": "Missing Telegram config" + (", auto-approving" if auto_execute else ", blocking (auto_execute=false)")}
        _log_approval({"ts": ts, "order": _safe_order_summary(order_info), **result})
        return result
    ...
    if msg_id is None:
        result = {"approved": auto_execute, "decision": "error", "reason": "Failed to send Telegram message" + (", auto-approving" if auto_execute else ", blocking (auto_execute=false)")}
        _log_approval({"ts": ts, "order": _safe_order_summary(order_info), **result})
        return result
    ...
    if decision == "approved":
        ...
        result = {"approved": auto_execute, "decision": "error", "reason": f"Unknown decision: {decision}" + (", auto-approving" if auto_execute else ", blocking")}
