...
def auth_headers(token, token_type="session"):
...
def step1_challenge():
    ...
    r = requests.post(f"{API_URL}/sessions", json={"login": username, "password": password, "remember-me": True},
...
def test_connection():
    ...
    r = requests.get(f"{API_URL}/customers/me", headers=auth_headers(token, token_type))
    ...
    r = requests.get(f"{API_URL}/customers/me/accounts", headers=auth_headers(token, token_type))
    ...
    if r.status_code == 200:
        ...
        for acct in accounts:
            ...
            r3 = requests.get(f"{API_URL}/accounts/{num}/balances", headers=auth_headers(token, token_type))
...
def renew_session():
    ...
    if not session_data:
        session_data = {}
    remember_token = session_data.get("remember_token") or env.get("TASTYTRADE_REMEMBER_TOKEN", "")
    ...
    r = requests.post(f"{API_URL}/sessions", json={
        "login": username, "remember-token": remember_token
    }, headers={"Content-Type": "application/json"})
