...
ssh "$VM_HOST" "cd $VM_DIR && tar czf /tmp/gs_backup_${TIMESTAMP}.tar.gz config/ .env 2>/dev/null || true"
...
ssh "$VM_HOST" "cd $VM_DIR && mkdir -p src/policy src/data data/pit reports/operational"

# Step 4: Install new pip dependencies (if any)
echo "[4/7] Installing dependencies..."
ssh "$VM_HOST" "cd $VM_DIR && pip3 install pyyaml --quiet 2>/dev/null || true"
