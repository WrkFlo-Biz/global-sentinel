...
def check_services() -> Dict[str, Any]:
    ...
    for svc in CRITICAL_SERVICES:
        out, _rc = run(f"systemctl is-active {svc}")
        status = out.strip()
        results[svc] = {
            "status": status,
...
def _bridge_refresh_candidates(freshness: Dict[str, Any]) -> List[str]:
    candidates: List[str] = []
    for bridge in freshness.get("stale_bridges", []) or []:
        fname = bridge.get("file")
        if fname:
            candidates.append(str(fname))
    for fname in freshness.get("missing_bridges", []) or []:
        candidates.append(str(fname))
...
def refresh_stale_bridges(freshness: Dict[str, Any]) -> List[str]:
    """Best-effort refresh for known stale/missing bridges."""
    alerts: List[str] = []

    for fname in _bridge_refresh_candidates(freshness):
        cmd = BRIDGE_REFRESH_CMDS.get(fname)
        if not cmd:
            continue

        fpath = QF / fname
        before_exists = fpath.exists()
        before_mtime = fpath.stat().st_mtime if before_exists else None
        ...
        try:
            ...
            detail = f"{fname}: refresh error {exc}"
            log_recovery("bridge_refresh", detail, False)
            alerts.append(f"FAILED bridge refresh {fname}")
            continue

        after_exists = fpath.exists()
        after_mtime = fpath.stat().st_mtime if after_exists else None
        ...
        if result.returncode == 0 and wrote_file:
            detail = f"{fname}: refreshed via {cmd}"
            log_recovery("bridge_refresh", detail, True)
            alerts.append(f"Refreshed stale bridge {fname}")
            continue

        stderr_tail = (result.stderr or "")[-240:].strip().replace("\n", " | ")
        stdout_tail = (result.stdout or "")[-240:].strip().replace("\n", " | ")
        detail = (
            f"{fname}: rc={result.returncode}, wrote_file={wrote_file}, "
...
def check_telegram() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"connected": False, "reason": str(exc)[:100]}
...
def get_historical_score(hours_ago: float) -> Optional[int]:
    ...
    best_diff = float("inf")
    try:
        with open(HEALTH_HISTORY_FILE, encoding="utf-8") as handle:
            for line in handle:
                line = line.strip()
                if not line:
                    continue
...
def save_health_history(score: int):
    ...
    with open(HEALTH_HISTORY_FILE, "a", encoding="utf-8") as handle:
        handle.write(json.dumps(entry) + "\n")

    try:
        lines = HEALTH_HISTORY_FILE.read_text(encoding="utf-8").strip().splitlines()
        if len(lines) > 600:
...
def get_restart_count(svc: str, window_seconds: int = 3600) -> int:
    ...
    count = 0
    try:
        with open(RECOVERY_LOG, encoding="utf-8") as handle:
            for line in handle:
                line = line.strip()
                if not line:
                    continue
...
def auto_recover_services(services: Dict[str, Any]) -> List[str]:
    ...
    for svc, info in services.items():
        ...
        if restart_count >= 3:
            msg = (
                f"Service {svc} has restarted {restart_count} times in 1 hour - "
                "disabling to prevent loop"
            )
            alerts.append(msg)
...
def emergency_disk_cleanup(disk_pct: int) -> List[str]:
    ...
    cleanups = [
        (
            f'find {LOGS}/ -name "*.log" -size +50M -exec truncate -s 5M {{}} \\;',
            "truncate logs >50MB",
        ),
        (
            f"find {LOGS}/ -name \"*.jsonl\" -size +50M -exec sh -c 'tail -2000 \"$1\" > \"$1.tmp\" && mv \"$1.tmp\" \"$1\"' _ {{}} \\;",
            "trim jsonl >50MB",
        ),
...
def emergency_swap_cleanup(resources: Dict[str, Any]) -> List[str]:
    ...
    for line in out.splitlines()[1:]:
        ...
        if len(cols) > 10:
            ...
            if mem_pct > 10 and not any(pattern in cmd for pattern in critical_patterns):
...
def main():
    ...
    bridge_refresh_alerts = refresh_stale_bridges(freshness)
    if bridge_refresh_alerts:
        recovery_alerts.extend(bridge_refresh_alerts)
        freshness = check_data_freshness()

    dead_services = [svc for svc, info in services.items() if not info.get("active")]
    if dead_services:
    ...
    HEALTH_FILE.write_text(json.dumps(health_data, indent=2, default=str), encoding="utf-8")
