...
def send_telegram_alert(message):
    ...
    if _send_topic:
        try:
            _send_topic(
                message[:4000] if isinstance(message, str) else str(message)[:4000],
                topic="system",
            )
            return
        except Exception:
    ...
    for filename, ttl in ttls.items():
        ...
        if filename in BRIDGE_RERUN:
            ...
            try:
                ...
                rerun_fail.append(filename)
                log(f"Bridge {filename} re-run error: {exc}")
...
def _alert_signature(failed_svcs, stale_bridges):
    stale_names = sorted([name for (name, _age) in stale_bridges])
...
def main():
    ...
    log(
        f"Bridges: {len(stale_bridges)} stale, {len(rerun_ok)} re-run OK, {len(rerun_fail)} re-run failed"
    )
