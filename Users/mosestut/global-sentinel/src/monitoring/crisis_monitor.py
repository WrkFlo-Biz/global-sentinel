...
class CrisisMonitor:
    ...
    def __init__(self, repo_root: Path):
        ...
        # Telegram command handlers disabled — OpenClaw gateway handles bot
        # commands. Running getUpdates here conflicts with the gateway (409).
    ...
    def _check_feature_freshness(self, bridge_results: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {"error": str(e), "max_confidence_penalty": 0}
        ...
        try:
            ...
            if high_conv and self.alerter:
                ...
                for d in high_conv:
                    ...
                    conf = float(d.get("confidence", 0.0))
                    src = d.get("source", "")
                    hc_lines.append(f"  {sym} ({cat}) {action} {conf:.2f} [{src}]")
                hc_body = "\n".join(hc_lines)
                try:
                    self.alerter._dispatch(
            ...
            if not result.ok:
                # Fallback: send directly via alerter if topic routing fails
                try:
